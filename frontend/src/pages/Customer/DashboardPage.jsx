...
import { FiActivity, FiDollarSign, FiFileText, FiAlertCircle, FiArrowUpRight, FiClock, FiCheckCircle } from "react-icons/fi";
...
const StatCard = ({ title, value, icon: Icon, trend, trendUp }) => (<div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm transition-all duration-300 hover:shadow-md">
    <div className="flex items-center justify-between mb-4">
      <div className="p-3 bg-indigo-50 dark:bg-neutral-800 rounded-lg text-indigo-600 dark:text-indigo-400">
        <Icon size={24}/>
      </div>
      {trend && (<span className={`flex items-center text-xs font-medium ${trendUp ? 'text-green-600 dark:text-green-400' : 'text-red-600'}`}>
...
const StatusBadge = ({ status }) => {
    ...
    return (<span className={`px-3 py-1 rounded-full text-xs font-medium border border-transparent ${badgeStyle}`}>
      {status}
    </span>);
};
// --- Main Component ---
    ...
    return (<DashboardLayout>
      <div className="p-4 md:p-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Active Policies" value="4" icon={FiFileText} trend="+1 New" trendUp={true}/>
          <StatCard title="Total Claims" value="12" icon={FiActivity}/>
          <StatCard title="Pending Actions" value="1" icon={FiAlertCircle} trend="Action Req" trendUp={false}/>
          <StatCard title="Total Coverage" value="$1.2M" icon={FiDollarSign}/>
        </section>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-neutral-800">
                  {recentClaims.map((claim) => (<tr key={claim.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800/30 transition-colors">
