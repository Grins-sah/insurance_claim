...
import { FiBell, FiFileText, FiAlertCircle, FiCheckCircle, FiMail, FiClock, FiX } from "react-icons/fi";
...
const NotificationCard = ({ notification }) => {
    ...
    return (<a href={notification.link || '#'} className={`flex items-start p-4 rounded-xl border border-gray-200 dark:border-neutral-800 transition-all duration-200 ${backgroundClasses} ${notification.link ? 'cursor-pointer' : 'cursor-default'}`}>
...
const NotificationsPage = () => {
    ...
    const filteredNotifications = mockNotifications.filter(notif => {
        ...
        if (filter === 'unread')
            return !notif.isRead;
        if (filter === 'action')
            return notif.type === 'Action Required';
        return false;
    });
    const tabClasses = (tabName) => `px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 
     ${filter === tabName
    ...
    return (<DashboardLayout>
      <div className="p-4 md:p-8">
        ...
        <div className="space-y-4">
          {filteredNotifications.length > 0 ? (filteredNotifications.map(notif => (<NotificationCard key={notif.id} notification={notif}/>))) : (<div className="text-center py-10 text-gray-500 dark:text-neutral-500">
