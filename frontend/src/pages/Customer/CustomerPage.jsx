...
const CustomerPage = () => {
    ...
    return (<DashboardLayout userName={name}>
      ...
      <section className="space-y-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {claims.map((c) => (<tr key={c.id} className="border-t">
                <td>{c.id}</td>
                <td>{c.policyId}</td>
                <td>{c.type}</td>
                <td>{c.date}</td>
      ...
      <CreateClaimModal open={showModal} loading={loading} onClose={() => setShowModal(false)} onSubmit={submitClaim} claimType={claimType} setClaimType={setClaimType} description={description} setDescription={setDescription}/>
    </DashboardLayout>);
};
