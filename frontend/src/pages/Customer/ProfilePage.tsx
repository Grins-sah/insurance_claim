...
const DetailItem: React.FC<DetailItemProps> = ({ icon: Icon, label, value, isEditable = false, onEdit }) => (
    <div className="flex justify-between items-center py-3 border-b border-gray-100 dark:border-neutral-800">
        ...
        {isEditable && (
            <button 
                onClick={onEdit} 
                className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 flex items-center gap-1 transition"
            >
                <FiEdit size={14} /> Edit
            </button>
        )}
            ...
            <div className="p-4 md:p-8">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    {isEditing ? (
                        <button 
                            onClick={handleSave}
                            className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-medium transition shadow-md"
                        >
                            <FiSave /> Save Changes
                        </button>
                    ) : (
                        <button 
                            ...
                            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-medium transition shadow-md"
                        >
                            <FiEdit /> Edit Profile
                        </button>
                    )}
                        ...
                        <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                {isEditing ? (
                                    <div className="py-3 border-b border-gray-100 dark:border-neutral-800">
                                        ...
                                        <input 
                                            ...
                                            className="w-full px-3 py-2 bg-gray-50 dark:bg-neutral-800 border border-gray-300 dark:border-neutral-700 rounded-lg text-sm text-gray-900 dark:text-neutral-200"
                                        />
                                    </div>
                                ) : (
                                    <DetailItem icon={FiMail} label="Email Address" value={profile.email} />
                                )}
                                ...
                                {isEditing ? (
                                    ...
                                    <DetailItem icon={FiPhone} label="Phone Number" value={profile.phone} />
                                )}
                            </div>
                        </div>
