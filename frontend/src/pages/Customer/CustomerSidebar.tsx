...
import {
  FiGrid, FiFileText, FiPlusCircle, FiBell, FiUser, FiSettings,
} from "react-icons/fi";
...
const CustomerSidebar = () => {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 min-h-screen bg-neutral-900 text-neutral-100 border-r border-neutral-800">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.path} // 👈 Use Link to navigate without refreshing the page
              className={`
